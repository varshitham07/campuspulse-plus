-- CampusPulse+ Production Schema (Aiven MySQL Compatible)
-- MySQL 8.0.28+
-- Corrected: CAMPUS STRUCTURE tables created FIRST to satisfy foreign key constraints

CREATE DATABASE IF NOT EXISTS campuspulse CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE campuspulse;

-- ============ CAMPUS STRUCTURE (MUST BE FIRST) ============
...
-- ============ USERS (MUST BE SECOND — no dept/class FK) ============
CREATE TABLE users (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE club_assignments (
  ...
  INDEX idx_user_role (user_id, assignment_role)
);

-- ============ CLUB TOOLS: FORMS, MEDIA, LINKS ============
...
FROM departments d CROSS JOIN (
  SELECT 1 year_of_study,'A' section,'1st Year' label UNION ALL 
  SELECT 1,'B','1st Year' UNION ALL 
  SELECT 2,'A','2nd Year' UNION ALL 
  SELECT 3,'A','3rd Year' UNION ALL 
  SELECT 4,'A','4th Year'
) y;